...
class ProgramFlowMixin:
    def checkProgram(self, programNode: Any) -> None:
        ...
        if entryRite is not None:
            ...
            self.checkGenesisRules(entryRite)
        else:
            self.addError(
                programNode,
                "Program must contain exactly one genesis() rite."
            )
    ...
    def checkGenesisRules(self, riteNode: Any) -> None:
        ...
        riteSymbol = self.functions.get("genesis")

        if riteSymbol is None:
            self.addError(
                riteNode,
        ...
        returnType = riteSymbol.returnType

        if not (
            returnType.isBaseType(BaseType.TALLY)
            or returnType.isBaseType(BaseType.HOLLOW)
        ):
            self.addError(
                riteNode,
                "genesis() may only return tally or hollow."
            )
            return

        dismissStatement = getattr(riteNode, "dismissStatement", None)

        # fallback: parser may have consumed final dismiss as a normal body statement
        if dismissStatement is None:
            bodyStatements = getattr(riteNode, "bodyStatements", []) or []

            if bodyStatements:
                lastStatement = bodyStatements[-1]

                if getClassName(lastStatement) == "DismissStatement":
                    dismissStatement = lastStatement

        if returnType.isBaseType(BaseType.HOLLOW):
            if dismissStatement is not None:
                dismissValue = getattr(dismissStatement, "value", None)

                if dismissValue is not None:
                    self.addError(
                        dismissStatement,
                        "hollow genesis() cannot dismiss a value."
                    )

            return

        # tally genesis must end with literal dismiss 0;
        if dismissStatement is None:
            self.addError(
                riteNode,
                "tally genesis() must end with dismiss 0;"
            )
            return

        dismissValue = getattr(dismissStatement, "value", None)

        if getClassName(dismissValue) != "LiteralExpression":
            self.addError(
                dismissStatement,
                "tally genesis() must dismiss literal 0."
            )
            return

        literalType = (getattr(dismissValue, "literalType", "") or "").lower()
        literalValue = getattr(dismissValue, "value", None)

        if literalType != "int" or literalValue != 0:
            self.addError(
                dismissStatement,
                "tally genesis() must dismiss literal 0."
            )
                ...
                for parameterSymbol in riteSymbol.parameters:
                    if parameterSymbol.name in seenParameterNames:
                        self.addError(
                            ...
                            f"Rite '{riteName}' must dismiss a value of type {self.getTypeName(riteSymbol.returnType)}."
                        )
